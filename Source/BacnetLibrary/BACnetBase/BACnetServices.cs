...
namespace BacnetLibrary.BACnetBase
{
    public class Services
    {
        ...
        public static void EncodeLogRecord(EncodeBuffer buffer, BacnetLogRecord record)
        {
            ...
            /* Tag 1: logDatum */
            if (record.type != BacnetTrendLogValueTypes.TL_TYPE_NULL)
            {

                if (record.type == BacnetTrendLogValueTypes.TL_TYPE_ERROR)
                {
                    ASN1.encode_opening_tag(buffer, 1);
                    ASN1.encode_opening_tag(buffer, 8);
        ...
        public static int DecodeLogRecord(byte[] buffer, int offset, int length, int n_curves, out BacnetLogRecord[] records)
        {
            ...
            for (int CurveNumber = 0; CurveNumber < n_curves; CurveNumber++)
            {
                ...
                records[CurveNumber].type = (BacnetTrendLogValueTypes)ContextTagType;

                switch ((BacnetTrendLogValueTypes)ContextTagType)
                {
                    case BacnetTrendLogValueTypes.TL_TYPE_STATUS:
