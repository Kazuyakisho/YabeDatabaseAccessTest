...
namespace BacnetLibrary.BACnetBase.BACnetSerialize
{
    public class ASN1
    {
        ...
        {
            encode_tag(buffer, (byte)BacnetApplicationTags.BACNET_APPLICATION_TAG_BOOLEAN, false, boolean_value ? 1 : (uint)0);
        }
        ...
        public static void encode_read_access_result(EncodeBuffer buffer, BacnetReadAccessResult value)
        {
            ...
            foreach (BacnetPropertyValue p_value in value.values)
            {
                ...
                encode_context_enumerated(buffer, 2, p_value.property.propertyIdentifier);
                /* Tag 3: optional propertyArrayIndex */
                if (p_value.property.propertyArrayIndex != BACNET_ARRAY_ALL)
                    encode_context_unsigned(buffer, 3, p_value.property.propertyArrayIndex);

                if (p_value.value != null && p_value.value is IList<BacnetError>)
                {
                    /* Tag 5: Error */
                    encode_opening_tag(buffer, 5);
                    encode_application_enumerated(buffer, (uint)((IList<BacnetError>)p_value.value)[0].error_class);
                    encode_application_enumerated(buffer, (uint)((IList<BacnetError>)p_value.value)[0].error_code);
        ...
        public static int decode_read_access_result(byte[] buffer, int offset, int apdu_len, out BacnetReadAccessResult value)
        {
            ...
            while ((apdu_len - len) > 0)
            {
                ...
                if (tag_number == 4)
                {
                    ...
                    {
                        tag_len = bacapp_decode_application_data(buffer, offset + len, apdu_len + offset - 1, value.objectIdentifier.type, (BacnetPropertyIds)new_entry.property.propertyIdentifier, out v);
        ...
        public static int decode_read_access_specification(byte[] buffer, int offset, int apdu_len, out BacnetReadAccessSpecification value)
        {
            ...
            while ((apdu_len - len) > 1 && !decode_is_closing_tag_number(buffer, offset + len, 1))
            {
                ...
                if ((len + len_value_type) >= apdu_len)
                    return -1;
                len += decode_enumerated(buffer, offset + len, len_value_type, out p_ref.propertyIdentifier);
                ...
                {
                    tmp = decode_tag_number_and_value(buffer, offset + len, out tag_number, out len_value_type);
                    if (tag_number == 1)
                    {
                        len += tmp;
                        ...
                        if ((len + len_value_type) >= apdu_len)
                            return -1;
                        len += decode_unsigned(buffer, offset + len, len_value_type, out p_ref.propertyArrayIndex);
                    }
                }
        ...
        public static int decode_device_obj_property_ref(byte[] buffer, int offset, int apdu_len, out BacnetDeviceObjectPropertyReference value)
        {
            ...
            {
                len += tag_len;
                len += decode_unsigned(buffer, offset + len, len_value_type, out value.arrayIndex);
            }
            ...
            len += decode_object_id(buffer, offset + len, out value.deviceIndentifier.type, out value.deviceIndentifier.instance);
        ...
        public static bool decode_is_context_tag_with_length(byte[] buffer, int offset, byte tag_number, out int tag_length)
        {
            ...
            return IS_CONTEXT_SPECIFIC(buffer[offset]) &&
        ...
        public static int bacapp_decode_context_application_data(byte[] buffer, int offset, int max_offset, BacnetObjectTypes object_type, BacnetPropertyIds property_id, out BacnetValue value)
        {
            ...
            if (IS_CONTEXT_SPECIFIC(buffer[offset]))
            {
                ...
                if (property_id == BacnetPropertyIds.PROP_ACTIVE_COV_SUBSCRIPTIONS)
                {
                ...
                if (object_type == BacnetObjectTypes.OBJECT_GROUP && property_id == BacnetPropertyIds.PROP_PRESENT_VALUE)
                {
                ...
                if ((property_id == BacnetPropertyIds.PROP_LIST_OF_OBJECT_PROPERTY_REFERENCES) || (property_id == BacnetPropertyIds.PROP_LOG_DEVICE_OBJECT_PROPERTY))
                {
                ...
                if (property_id == BacnetPropertyIds.PROP_DATE_LIST)
                {
        ...
        public static bool decode_is_context_tag(byte[] buffer, int offset, byte tag_number)
        {
            ...
            return IS_CONTEXT_SPECIFIC(buffer[offset]) && (my_tag_number == tag_number);
        }
        ...
        public static bool decode_is_opening_tag_number(byte[] buffer, int offset, byte tag_number)
        {
            ...
            decode_tag_number(buffer, offset, out my_tag_number);
            return IS_OPENING_TAG(buffer[offset]) && (my_tag_number == tag_number);
        }
        ...
        public static bool decode_is_closing_tag_number(byte[] buffer, int offset, byte tag_number)
        {
            ...
            decode_tag_number(buffer, offset, out my_tag_number);
            return IS_CLOSING_TAG(buffer[offset]) && (my_tag_number == tag_number);
        }
