...
namespace BacnetLibrary.BACnetClient
{
    public class BacnetClient : IDisposable
    {
        ...
        private bool m_force_window_size;
        private uint m_writepriority;
        ...
        {
            m_client.Start();
            m_client.MessageRecieved += OnRecieve;
            Trace.TraceInformation("Started communication");
        }

        public delegate void ConfirmedServiceRequestHandler(BacnetClient sender, BacnetAddress adr, BacnetPduTypes type, BacnetConfirmedServices service, BacnetMaxSegments max_segments, BacnetMaxApdu max_adpu, byte invoke_id, byte[] buffer, int offset, int length);
        ...
        protected void ProcessConfirmedServiceRequest(BacnetAddress adr, BacnetPduTypes type, BacnetConfirmedServices service, BacnetMaxSegments max_segments, BacnetMaxApdu max_adpu, byte invoke_id, byte[] buffer, int offset, int length)
        {
            try
            {
        ...
        public delegate void SegmentHandler(BacnetClient sender, BacnetAddress adr, BacnetPduTypes type, BacnetConfirmedServices service, byte invoke_id, BacnetMaxSegments max_segments, BacnetMaxApdu max_adpu, byte sequence_number, bool first, bool more_follows, byte[] buffer, int offset, int length);
        public event SegmentHandler OnSegment;

        private void ProcessSegment(BacnetAddress adr, BacnetPduTypes type, BacnetConfirmedServices service, byte invoke_id, BacnetMaxSegments max_segments, BacnetMaxApdu max_adpu, bool server, byte sequence_number, byte proposed_window_number, byte[] buffer, int offset, int length)
        {
            bool first = false;
        ...
        /// </summary>
        private void PerformDefaultSegmentHandling(BacnetClient sender, BacnetAddress adr, BacnetPduTypes type, BacnetConfirmedServices service, byte invoke_id, BacnetMaxSegments max_segments, BacnetMaxApdu max_adpu, byte sequence_number, bool first, bool more_follows, byte[] buffer, int offset, int length)
        {
            if (first)
            {
                ...
                case BacnetPduTypes.PDU_TYPE_COMPLEX_ACK:
                    {
                        ...
                        {
                            ProcessSegment(adr, type, service, invoke_id, BacnetMaxSegments.MAX_SEG0, BacnetMaxApdu.MAX_APDU50, false, sequence_number, proposed_window_number, buffer, offset, length);
                        }
                    }
                    break;
        ...
        public void RegisterAsForeignDevice(String BBMD_IP, short TTL, int Port = 0xbac0)
        {
            ...
            try
            {
                IPEndPoint ep = new IPEndPoint(IPAddress.Parse(BBMD_IP), Port);
        ...
        public int GetMaxApdu()
        {
            ...
            {
                case BacnetMaxApdu.MAX_APDU1476:
                    max_apdu = 1476;
                    break;
                case BacnetMaxApdu.MAX_APDU1024:
                    max_apdu = 1024;
                    break;
                case BacnetMaxApdu.MAX_APDU480:
                    max_apdu = 480;
                    break;
                case BacnetMaxApdu.MAX_APDU206:
                    max_apdu = 206;
                    break;
                case BacnetMaxApdu.MAX_APDU128:
                    max_apdu = 128;
                    break;
                case BacnetMaxApdu.MAX_APDU50:
                    max_apdu = 50;
                    break;
                default:
        ...
        public IAsyncResult BeginWriteFileRequest(BacnetAddress adr, BacnetObjectId object_id, int position, int count, byte[] file_buffer, bool wait_for_transmit, byte invoke_id = 0)
        {
            ...
            Services.EncodeAtomicWriteFile(b, true, object_id, position, 1, new[] { file_buffer }, new[] { count });
        ...
        public IAsyncResult BeginRemoveListElementRequest(BacnetAddress adr, BacnetObjectId object_id, BacnetPropertyReference reference, IList<BacnetValue> value_list, bool wait_for_transmit, byte invoke_id = 0)
        {
            ...
            Services.EncodeAddListElement(b, object_id, reference.propertyIdentifier, reference.propertyArrayIndex, value_list);
            ...
            using (BacnetAsyncResult result = (BacnetAsyncResult)BeginGetAlarmSummaryOrEventRequest(adr, GetEvent, Alarms, true, invoke_id))
            {
                for (int r = 0; r < m_retries; r++)
                {
                    if (result.WaitForDone(m_timeout))
                    {
                        ...
                        if (ex != null) return false;
                        if (MoreEvent) // never true if GetAlarmSummary is used
        ...
        public void ReadFileResponse(BacnetAddress adr, byte invoke_id, Segmentation segmentation, int position, uint count, bool end_of_file, byte[] file_buffer)
        {
            ...
            {
                Services.EncodeAtomicReadFileAcknowledge(b, true, end_of_file, position, 1, new[] { file_buffer }, new[] { (int)count });
            });
        }
