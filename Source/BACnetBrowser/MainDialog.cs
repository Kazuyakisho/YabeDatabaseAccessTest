...
using BacnetLibrary.BACnetDeviceStorage;
using BacnetLibrary.BACnetFile;
using BacnetLibrary.BACnetTransport;
...
namespace BACnetBrowser
{
    public partial class MainDialog : Form
    {
        ...
        private Dictionary<string, Subscription> subTest = new Dictionary<string, Subscription>();


        //nested classes

        #region "Bacnet Device Line "
        ...
        {
            InitializeComponent();
            DBManager.Instance.daysOfWeek.SetWeekdays(Settings.Default.SQLDataUploadWeekDays);
            System.Diagnostics.Trace.Listeners.Add(new MyTraceListener(this));
            ...
            lock (m_subscription_list)
                if (m_subscription_list.ContainsKey(sub_key))
                {
                    this.BeginInvoke((MethodInvoker)delegate
                    {
                            ...
                            foreach (BacnetPropertyValue value in values)
                            {
                                switch ((BacnetPropertyIds)value.property.propertyIdentifier)
                                {
                                    case BacnetPropertyIds.PROP_PRESENT_VALUE:
                                        itm.SubItems[3].Text = ConvertString.ConvertToText(value.value);
                        ...
                        {
                            System.Diagnostics.Trace.TraceError("Exception in subcribed value: " + ex.Message);
                        }
                    });
                }
        ...
        private void MainDialog_Load(object sender, EventArgs e)
        {
            ...
            CustomSingleConverter.DontDisplayExactFloats = true;
        }
                        ...
                        if (bobj_id.type == BacnetObjectTypes.OBJECT_DEVICE)
                        {
                            if (e.Node.ToolTipText == "")   // already update with the device name
                            {
                                ...
                                if (Prop_Object_NameOK)
                                {
                                    ...
                                    _DeviceName = e.Node.Text = Identifier + " [" + device_id.ToString() + "] ";
                                    ...
                                    try
                                    {
                                        ...
                                        if (comm.ReadPropertyRequest(adr, bobj_id, BacnetPropertyIds.PROP_OBJECT_NAME, out values))
                                        {
                                            ...
                                            _DeviceName = e.Node.Text = values[0].ToString() + " [" + device_id.ToString() + "] ";  // change @ by the Name    
                                            lock (DevicesObjectsName)
                                            {
        ...
        private bool ReadProperty(BacnetClient comm, BacnetAddress adr, BacnetObjectId object_id, BacnetPropertyIds property_id, ref IList<BacnetPropertyValue> values, uint array_index = ASN1.BACNET_ARRAY_ALL)
        {
        ...
        private bool ReadAllPropertiesBySingle(BacnetClient comm, BacnetAddress adr, BacnetObjectId object_id, out IList<BacnetReadAccessResult> value_list)
        {
            ...
            try
            {
                ...
                BacnetPropertyValue new_entry = new BacnetPropertyValue();
                new_entry.property = new BacnetPropertyReference((uint)BacnetPropertyIds.PROP_OBJECT_IDENTIFIER, ASN1.BACNET_ARRAY_ALL);
                ...
                new_entry.property = new BacnetPropertyReference((uint)BacnetPropertyIds.PROP_OBJECT_TYPE, ASN1.BACNET_ARRAY_ALL);
        ...
        private void UpdateGrid(TreeNode selected_node)
        {
            ...
            try
            {
                ...
                if (selected_node.Tag is BacnetObjectId)
                {
                    ...
                    BacnetPropertyReference[] properties = new BacnetPropertyReference[] { new BacnetPropertyReference((uint)BacnetPropertyIds.PROP_ALL, ASN1.BACNET_ARRAY_ALL) };
                    ...
                    //update grid
                    DynamicPropertyGridContainer bag = new DynamicPropertyGridContainer();
                    foreach (BacnetPropertyValue p_value in multi_value_list[0].values)
                    {
                        ...
                        switch ((BacnetPropertyIds)p_value.property.propertyIdentifier)
                        {
                            ...
                            case BacnetPropertyIds.PROP_PRESENT_VALUE:
                                ...
                                bag.Add(new CustomProperty(GetNiceName((BacnetPropertyIds)p_value.property.propertyIdentifier), value, t != null ? t : typeof(string), false, "", b_values.Length > 0 ? b_values[0].Tag : (BacnetApplicationTags?)null, null, p_value.property));
                                break;

                            default:
                                bag.Add(new CustomProperty(GetNiceName((BacnetPropertyIds)p_value.property.propertyIdentifier), value, value != null ? value.GetType() : typeof(string), false, "", b_values.Length > 0 ? b_values[0].Tag : (BacnetApplicationTags?)null, null, p_value.property));
                                break;
                        }
        ...
        private void m_DataGrid_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            ...
            try
            {
                ...
                CustomPropertyDescriptor c = null;
                GridItem gridItem = e.ChangedItem;
                // Go up to the Property (could be a sub-element)

                do
                {
                    if (gridItem.PropertyDescriptor is CustomPropertyDescriptor)
                        c = (CustomPropertyDescriptor)gridItem.PropertyDescriptor;
                    else
        ...
        private void CreateSubscription(BacnetClient comm, BacnetAddress adr, uint device_id, BacnetObjectId object_id)
        {
            ...
            try
            {
                ...
                if (device_id >= ASN1.BACNET_MAX_INSTANCE)
                {
                ...
                //add to list
                ListViewItem itm = m_SubscriptionView.Items.Add(m_AddressSpaceTree.Nodes[0].Text + " - " + adr + " - " + device_id);
                ...
                itm.SubItems.Add("No"); //upload to db
                //add to index
        ...
        private void m_SubscriptionView_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (m_SubscriptionView.SelectedItems.Count == 1)
            {
                ...
                if (itm.Tag is Subscription)    // It's a subscription or not (Event/Alarm)
                {
                    ...
                    string sub_key = itm.SubItems[0].Text + " - " + itm.SubItems[1].Text;
                    DBManager.Instance.RemoveCovDataItem(sub_key);
        ...
        {
            IList<BacnetValue> value;
            if (comm.ReadPropertyRequest(adr, new BacnetObjectId(BacnetObjectTypes.OBJECT_DEVICE, ASN1.BACNET_MAX_INSTANCE), BacnetPropertyIds.PROP_OBJECT_IDENTIFIER, out value))
            {
                ...
                BacnetPropertyReference[] properties = new BacnetPropertyReference[2]
                                                                {
                                                                    ...
                                                                    new BacnetPropertyReference((uint)BacnetPropertyIds.PROP_DESCRIPTION, ASN1.BACNET_ARRAY_ALL),
                                                                };
        ...
        private void databaseSettingstoolstrip_Click(object sender, EventArgs e)
        {

            DatabaseConnectionForm dlg = new DatabaseConnectionForm();
            dlg.ShowDialog(this);
        }

        private void m_subscribeToUploadDB_Opening(object sender, CancelEventArgs e)
        {


            //return if no items are selected
            if (m_SubscriptionView.SelectedItems.Count == 0)
            {
                e.Cancel = true;
                return;
            }

            //return if the selected item is not valid
            var item = m_SubscriptionView.SelectedItems[0];


            if (item.SubItems[5].Text == "Not started")
            {
                e.Cancel = true;
                item.SubItems[6].Text = DBServerAvailableEnum.No.ToString();
                return;
            }

            //if no subscribers are selected leave
            if (DBManager.Instance.DbSubsribers.Count == 0)
            {
                e.Cancel = true;
                item.SubItems[6].Text = DBServerAvailableEnum.NoServerAvailable.ToString();
                MessageBox.Show("Please add Connections to UploadList");
            ...
            //clear lists
            var stripMenuItemAdd = m_subscribeToUploadDB.Items[0] as ToolStripMenuItem;
            stripMenuItemAdd?.DropDownItems.Clear();

            var stripMenuItemRemove = m_subscribeToUploadDB.Items[1] as ToolStripMenuItem;
            stripMenuItemRemove?.DropDownItems.Clear();

            //Db Upload device covdata
            DBCovDataItem covData = null;

            //specify subkey for cov data
            string sub_key = item.SubItems[0].Text + " - " + item.SubItems[1].Text;


            //bool if cov exits
            bool isCovItem = false;

            //if cov already exits

            if (DBManager.Instance.DbCovDataItems.ContainsKey(sub_key))
            {
                covData = DBManager.Instance.DbCovDataItems[sub_key];
                isCovItem = true;
            }
            ...
            foreach (var dbSubscriber in DBManager.Instance.DbSubsribers)
            {

                //subscriber key is connection name
                string subscriber_key = dbSubscriber.Value.Prov.Name;

                Image image = null;



                //split first item into - DeviceName - Address - Device ID
                string[] splitFirstItem =
                    item.SubItems[0].Text.Replace(" ", String.Empty).Split(new[] { '-' });

                //if the subscriber is valid for uploading data add a picture
                if (DBManager.Instance.DbCovDataItems.ContainsKey(sub_key))
                    if (DBManager.Instance.DbCovDataItems[sub_key].Subscribers.ContainsKey(subscriber_key)) image = Properties.Resources.accept;

                //add to subscriber
                stripMenuItemAdd?.DropDownItems.Add(dbSubscriber.Value.Prov.Name, image,
                (o, args) =>
                {
                    if (isCovItem)
                    {
                        if (DBManager.Instance.DbCovDataItems[sub_key].Subscribers.ContainsKey(subscriber_key))
                        {
                            return;
                        }

                        DBManager.Instance.DbCovDataItems[sub_key].Subscribers.Add(subscriber_key, dbSubscriber.Value);
                    }
                    else
                    {

                        //initalize cov data item
                        covData = new DBCovDataItem(splitFirstItem[1], splitFirstItem[2], splitFirstItem[0], item.SubItems[1].Text,
                        item.SubItems[2].Text, "", DateTime.Now, item.SubItems[5].Text, true);


                        //Add covdata and add subscriber
                        DBManager.Instance.AddCovDataItem(sub_key, covData, subscriber_key);


                    }

                    item.SubItems[6].Text = DBServerAvailableEnum.Yes.ToString();

                    //upload timer
                    m_uploadDBTimer.Interval = Properties.Settings.Default.SQLDataUploadTimeSpan;
                    m_uploadDBTimer.Start();


                });

                // remove
                stripMenuItemRemove?.DropDownItems.Add(dbSubscriber.Value.Prov.Name, image, (o, args) =>
                {

                    //remove the subscriber
                    if (!DBManager.Instance.DbCovDataItems.ContainsKey(sub_key)) return;
                    if (DBManager.Instance.DbCovDataItems[sub_key].Subscribers.ContainsKey(subscriber_key))
                        DBManager.Instance.DbCovDataItems[sub_key].Subscribers.Remove(subscriber_key);


                    //if no subscribers are selected return
                    if (DBManager.Instance.DbCovDataItems[sub_key].Subscribers.Any(dbSubsriber => dbSubsriber.Value.IsValidSubscriber))
                    {
                        return;
                    }
            ...
            stripMenuItemRemove?.DropDownItems.Add("Delete All", null, (o, args) =>
            {

                if (DBManager.Instance.RemoveCovDataItem(sub_key))
                {
        ...
        private void toolStripMenuItemAddCovToDBUpload_Click(object sender, EventArgs e)
        {
        ...
        private void toolStripMenuItemRemoveCovFromDBUpload_Click(object sender, EventArgs e)
        {

        }

        private void toolStripMenuItemOpenDataBaseSettings_Click(object sender, EventArgs e)
        {
            DatabaseConnectionForm dlg = new DatabaseConnectionForm();
            dlg.ShowDialog(this);
        ...
        private void m_uploadDBTimer_Tick(object sender, EventArgs e)
        {

            if (DBManager.Instance.daysOfWeek.CheckTodayIsValid() == false)
                return;

            //if no subscribers available return error
            if (DBManager.Instance.DbSubsribers.Count == 0)
            {
                for (int i = 0; i < m_SubscriptionView.Items.Count; i++)
                {
                    m_SubscriptionView.Items[i].SubItems[6].Text = "NoServerAvailalbe";
                }

                //stop upload timer
                m_uploadDBTimer.Stop();
                return;
            }


            //if no cov data item is availaible do nothing
            if (DBManager.Instance.DbCovDataItems.Count != 0)
            {
                //else lock sub list and set the new values for uploading
                lock (m_subscription_list)
                {

                    foreach (var item in m_subscription_list.Values)
                    {
                        string sub_key = item.SubItems[0].Text + " - " + item.SubItems[1].Text;
                        ...
                        if (DBManager.Instance.DbCovDataItems.ContainsKey(sub_key))
                        {
                            try
                            {
                                DBManager.Instance.DbCovDataItems[sub_key].ObjectTime =
                                    Convert.ToDateTime(item.SubItems[4].Text);
                                DBManager.Instance.DbCovDataItems[sub_key].ObjectStatus = item.SubItems[5].Text;
                                DBManager.Instance.DbCovDataItems[sub_key].ObjectValue = item.SubItems[3].Text;
